...
import com.android.gotripmap.domain.entities.GreedySearchResults
...
class SearchMapV2(
  ...
  private fun findOptimalPathsForPedestrians(
    ...
    Log.w(
      "allPoints",
      nextPoints.joinToString { "${it.pointData.latitude} ${it.pointData.longitude} ${it.timeDeviation} ${it.optimalDistance}" })
    ...
    points.forEach {
      calculatePathParameters(
        ...
        if (calculated == points.size) {
          Log.w("pointsSize", points.size.toString())
          synchronisedQueue.put(result)
        }
      }
    }
    val results = synchronisedQueue.take()
    Log.w("resultPoints", results.joinToString { it.toString() })
    return results
  }
  ...
  private fun search(
    ...
    val searchListener = MapSearchListener { points ->
      thread {
        ...
        Log.w("data for index","$index ${path[index+1].pointData.longitude} ${path[index+1].pointData.latitude} ${timeRequirements[index + 1]} ${myPoints.joinToString { "${it.pointData.latitude} ${it.pointData.longitude}" }}")
        if (entry.transport == Transport.WALKING) {
          if (index == 0) {
        ...
        Log.w("intermediateResult", intermediateResults[index+1].joinToString { "${it.pointData.latitude} ${it.pointData.longitude}" })
        if (index == keywords.lastIndex) {
          val topResult = greedySearchForTopResults(intermediateResults[0][0], intermediateResults)
          Log.w(
            "topResult",
            topResult.joinToString { "${it.pointData.latitude} ${it.pointData.longitude}" })
          Log.w(
            "greedyResult",
            path.joinToString { "${it.pointData.latitude} ${it.pointData.longitude}" })
          if (index <= 1) {
            ...
            Log.w(
              "resultPath",
              pathResult.joinToString { "${it.pointData.longitude} ${it.pointData.latitude} +" })
    ...
    Thread.sleep(300+ Random.nextLong(250))
